...
int determinarDatosIniciais() 
{
    ...
    if((ptf = fopen("datos.dat","rb")) == NULL)
    {
        printf("Non se conseguiu abrir o arquivo (datos.dat)!\n");
        return (FRACASO);
    }
    
    while(feof(ptf) == 0) //Non chegamos o final do arquivo
    {
        fread(&temp,sizeof(datosCiclista),1,ptf);
        if(feof(ptf) == 0) //Evitamos contar un dato de máis
        {
            numDatos++;
        }
    }
...
int introducirDatosCiclistas(int numDatos)
{
    ...
    {
        if((ptf = fopen("datos.dat","wb")) == NULL)
        {
            printf("Non se conseguiu abrir o arquivo (datos.dat)!\n");
            return (FRACASO);
        }
    ...
    {
        if((ptf = fopen("datos.dat","ab")) == NULL)
        {
            printf("Non se conseguiu abrir o arquivo (datos.dat)!\n");
            return (FRACASO);
        }
    ...
    do //Introducimos os datos do cilista e valores nulos nos tempos das etapas
    {
        ...
        scanf("%s",temp.nome);
        fwrite(&temp, sizeof(datosCiclista), 1, ptf);
        numDatos++;
...
int introducirDatosEstrutura(datosCiclista *datos, int numDatos)
{
    ...
    if(numDatos != 0)
    {
        ...
        {
            fwrite(datos, sizeof(datosCiclista), numDatos, ptf);
        }
    }
...
int leerDatosCiclistas(datosCiclista *datos, int numDatos, int N)
{
    ...
    {
        fread(datos, sizeof(datosCiclista), numDatos, ptf);
    }
